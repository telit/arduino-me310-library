...
// Generic Modem Control -------------------------------------------------------
   ...
   _TEST(ims_pdp_apn_number_set,"AT#IMSPDPSET",TOUT_100MS) 
...
// DTE Modem Interface Control -------------------------------------------------
   ...
   _READ_TEST(dte_modem_local_control_flow,"AT+ICF",TOUT_100MS) 
...
// Network ---------------------------------------------------------------------
   ...
   _TEST(eps_network_registration_status,"AT+CEREG",TOUT_100MS) 
   ...
   _READ_TEST(cell_monitor,"AT#MONI",TOUT_100MS)
...
//   return_t read_iccid(tout_t aTimeout = TOUT_100MS) {return send_wait(F("AT#CCID"),OK_STRING,aTimeout);}
//   _TEST(read_iccid,"AT#CCID",TOUT_100MS) 
   ...
   _READ_TEST(restricted_sim_access,"AT+CRSM",TOUT_100MS) 
...
// SMS & CB --------------------------------------------------------------------
   ...
   return_t send_short_message(int length, tout_t aTimeout = TOUT_100MS);
   return_t send_short_message(const char *da, const char *toda, tout_t aTimeout = TOUT_100MS);
   ...
   return_t write_short_message_memory(int length, int stat, tout_t aTimeout = TOUT_100MS);
   return_t write_short_message_memory(const char *da, int toda, const char *stat, tout_t aTimeout = TOUT_100MS);
...
// Time & Alarm ----------------------------------------------------------------
   ...
   _READ_TEST(alarm_delete,"AT+CALD",TOUT_100MS) 
...
// Easy Scan -------------------------------------------------------------------
   ...
   _READ_TEST(network_survey,"AT#CSURV",TOUT_100MS) 
...
// IPEasy ----------------------------------------------------------------------
   ...
   _READ_TEST(socket_dial,"AT#SD",TOUT_100MS) 
      
   return_t socket_restore(int connId, tout_t aTimeout = TOUT_1SEC); 
   _READ_TEST(socket_restore,"AT#SO",TOUT_100MS) 

   return_t socket_shutdown(int connId, tout_t aTimeout = TOUT_1SEC); 
   _READ_TEST(socket_shutdown,"AT#SH",TOUT_100MS) 

   return_t socket_listen(int connId, int listenState, int listenPort, int lingerT, tout_t aTimeout = TOUT_1SEC); 
   ...
   _READ_TEST(socket_accept,"AT#SA",TOUT_100MS) 
      
   return_t socket_send_data_command_mode(int connId, int rai = 1, tout_t aTimeout = TOUT_1SEC); 
   _READ_TEST(socket_send_data_command_mode,"AT#SSEND",TOUT_100MS) 
      
   return_t socket_send_data_command_mode_extended(int connId, int bytesToSend, int rai = 1, tout_t aTimeout = TOUT_1SEC); 
   _READ_TEST(socket_send_data_command_mode_extended,"AT#SSENDEXT",TOUT_100MS) 
      
   return_t socket_receive_data_command_mode(int connId, int maxByte, int udpInfo = 0, tout_t aTimeout = TOUT_1SEC); 
   _READ_TEST(socket_receive_data_command_mode,"AT#SRECV",TOUT_100MS) 
   ...
   _READ_TEST(socket_send_udp_data_specific_remote_host,"AT#SSENDUDP",TOUT_100MS) 
   ...
   _READ_TEST(socket_send_udp_data_specific_remote_host_extended,"AT#SSENDUDPEXT",TOUT_100MS) 
      
   return_t socket_detect_cause_disconnection(int connId, tout_t aTimeout = TOUT_1SEC); 
   _READ_TEST(socket_detect_cause_disconnection,"AT#SLASTCLOSURE",TOUT_100MS) 
   ...
   _READ_TEST(dns_from_network,"AT#NWDNS",TOUT_100MS) 
...
// FTPEasy ---------------------------------------------------------------------
   ...
   return_t ftp_append(const char *fileName, int connMode = 0, tout_t aTimeout = TOUT_100MS);
   _READ_TEST(ftp_append,"AT#FTPAPP",TOUT_100MS) 

   return_t ftp_append_extended(int bytesToSend, int eof = 0, tout_t aTimeout = TOUT_100MS);
   _READ_TEST(ftp_append_extended,"AT#FTPAPPEXT",TOUT_100MS) 
   ...
   _TEST(ftp_size,"AT#FTPSIZE",TOUT_100MS) 
   ...
   _TEST(ftp_get_command_mode,"AT#FTPGETPKT",TOUT_100MS) 
   ...
   //int ftp_open(const char *server_port,const char *username, const char *password,int viewMode = 0, tout_t aTimeout = TOUT_100MS);
...
// SMTP ------------------------------------------------------------------------
   ...
   _READ_TEST(smtp_password,"AT#EPASSW",TOUT_100MS) 

   return_t smtp_mail_send(const char *da, const char *subj, tout_t aTimeout = TOUT_100MS);
   _READ_TEST(smtp_mail_send,"AT#EMAILD",TOUT_100MS) 
...
// HTTP ------------------------------------------------------------------------
   ...
   return_t send_http_send(int prof_id, int command, const char *resource, int data_len, const char *post_param ="", const char *extra_header_line = "",tout_t aTimeout = TOUT_100MS);
...
// SSL -------------------------------------------------------------------------  
   ...
   _READ_TEST(ssl_socket_open,"AT#SSLD",TOUT_100MS) 
   ...
   return_t ssl_socket_send_data(int ssid, int timeout = 100, tout_t aTimeout = TOUT_100MS);
   ...
   _TEST(ssl_security_data,"AT#SSLSECDATA",TOUT_100MS) 

   return_t ssl_additional_parameters(int ssid, int SNI = 0, tout_t aTimeout = TOUT_100MS);
   _TEST(ssl_additional_parameters,"AT#SSLSECCFG2",TOUT_100MS) 

      
// M2M -------------------------------------------------------------------------

   return_t m2m_chdir(const char *path,tout_t aTimeout = TOUT_100MS);
   _TEST(m2m_chdir,"AT#M2MCHDIR",TOUT_100MS) 
   ...
   _TEST(m2m_set_backup,"AT#M2MBACKUP",TOUT_100MS) 
   ...
   return_t m2m_write_file(const char *file_name, int size, int binToMod,tout_t aTimeout = TOUT_100MS);
...
// MQTT ------------------------------------------------------------------------
   ...
   _TEST(mqtt_read,"AT#MQREAD",TOUT_100MS) 
...
// -----------------------------------------------------------------------------      
   ...
   virtual void on_receive(void) {}                 //!< Callback function on string received
   ...
   virtual return_t wait_for(const char *aAnswer = OK_STRING, tout_t aTimeout = TOUT_100MS);
